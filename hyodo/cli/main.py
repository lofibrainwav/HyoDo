...
def admission_record(
    file: str | None = typer.Option(None, "--file", "-f", help="Path to one hyodo.admission-observation/v1 JSON object"),
    stdin_flag: bool = typer.Option(False, "--stdin", help="Read one admission observation from stdin"),
    root: str = typer.Option(".", "--root", help="Project root that owns .hyodo/admission-observations.jsonl"),
    ...
    try:
        text = sys.stdin.read() if stdin_flag else Path(file).read_text(encoding="utf-8")
