...
metadata: category: navigation-discovery author: AFO Kingdom map_version: "2025.01"
allowed-tools: - Read - Glob - Grep - Bash # MCP tools (optional - AFO Kingdom ) # - mcp__context7__*
standalone: true
--- # AFO Kingdom Navigator Your guide to navigating the AFO Kingdom codebase. Uses the royal map and compass to find any destination. ## Kingdom Structure ```
./
...
└── tests/ # Test suites
``` ## Quick Navigation Guide ### By Domain | Domain | Path | Key Files |
|--------|------|-----------|
...
| **MCP Servers** | `packages/trinity-os/trinity_os/servers/` | `*_mcp.py` | ### By Functionality | Need | Location |
|------|----------|
...
| Commands | `.claude/commands/*.md` | ### By File Type | File Type | Glob Pattern |
|-----------|--------------|
...
| Tests | `tests/**/*.py` | ## Reference Documents | Document | Path | Purpose |
|----------|------|---------|
...
| **SSOT** | `docs/AFO_FINAL_SSOT.md` | Single source of truth | ## Navigation Commands Use these to explore: ```bash
# Find all Trinity-related files
Glob: **/trinity*.py # Find Chancellor implementation
Grep: "class Chancellor" # Find API endpoints
Grep: "@router\." path:packages/afo-core/api # Find skill definitions
Glob: skills/*/SKILL.md
``` ## MCP Server Locations | Server | Path |
|--------|------|
...
| `memory` | External (npx) |
| `mcp-docker-gateway` | Docker (111 tools) | ## Port Map (Local Kingdom) | Service | Port | Package |
|---------|------|---------|
| Dashboard | 3000 | dashboard |
| Soul Engine API | 8010 | afo-core |
| PostgreSQL | 15432 | infrastructure |
| Redis | 6379 | infrastructure |
| Ollama | 11435 | infrastructure | ## Common Searches **"Where is the Trinity Score calculated?"**
→ `packages/trinity-os/trinity_os/servers/trinity_score_mcp.py` **"Where are the API routes?"**
→ `packages/afo-core/api/` **"Where is the dashboard home page?"**
→ `packages/dashboard/src/app/page.tsx` **"Where are the 3 strategists?"**
→ `packages/afo-core/AFO/chancellor/` **"Where is the Claude configuration?"**
→ `.claude/` (commands, agents, mcp.json) ## Pro Tips 1. Use `Glob` for pattern-based file discovery
