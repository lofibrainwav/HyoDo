...
## Step 3: Batch compatible candidates before integration

Parallelize research and dry-runs, but avoid merging a train of independent
PRs one at a time when they can safely be validated together. On a strict
up-to-date branch, each merge moves `main` and makes the remaining PR heads
behind; their old green checks cannot authorize the new base.

Use this bounded sequence:

```text
freeze one main SHA
  -> select one winner for each duplicate dependency/update
  -> prove candidates are pairwise compatible (write-sets + merge-tree)
  -> combine compatible changes on one clean integration branch
  -> run required checks once on that exact integration HEAD
  -> merge once, then perform one post-merge readback
```

Do not put conflicting candidates in the same batch. Keep a concrete conflict
as `BLOCK` with the conflicting paths and resolve it separately; do not
repeatedly retry an unchanged dirty candidate. Preserve superseded source
branches and close their PRs only after the selected replacement is merged and
read back.

If `main` changes after the batch base is frozen but before its checks finish,
stop that attempt. Rebuild the integration branch once from the new `main`,
re-run the dry-run, and bind all checks to the rebuilt HEAD. If `main` changes
again during that bounded attempt, report `HOLD` and coordinate the merge
window or adopt a merge queue; do not poll or rebase in an open-ended loop.

A merge queue is an alternative when configured: it validates a merge-group
SHA against the current base instead of asking authors to keep PR heads
rebased. GitHub Actions required workflows must subscribe to `merge_group` as
well as `pull_request` before the queue is enabled. Do not assume a queue is
available unless repository settings and a successful merge-group run prove
it.
...
- **Failed:** capture the failing step and minimal log evidence; fix only the
  owning lane.
- **Behind/unknown:** mark old checks stale, identify the changed base SHA,
  then rebuild or batch the candidate under the bounded integration sequence;
  never treat a stale green as current evidence.
