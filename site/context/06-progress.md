...
## 2026-09-06 — Fix round 1: docking mount-race after the motion.ts deferral

An independent judge reproduced a real regression from the JS-budget change above (PR #157, Finding A): making `initMotion` a dynamic import removed the previous guarantee that GSAP/ScrollTrigger was already listening before the graph module could mount and dispatch `eg-ready`. A visitor who scroll-jumps straight to the docking point in one step (not a gradual multi-tick scroll) could see the WebGL hero canvas fully opaque over the already-fully-mounted graph for up to ~1.7s, self-correcting only via GSAP's own deferred internal refresh rather than the app's own `eg-ready` re-measure path (which had already missed its one chance).

Two-part fix:
- `hero/motion.ts`: `initMotion()` now computes the current dock progress itself, synchronously, using the exact same formula as the `end` callback (factored into a shared `dockEnd()` function to avoid the two drifting apart), and calls `applyDock()` with it immediately on init — instead of relying solely on a future `eg-ready` event that may already have fired, or on `ScrollTrigger.create()`'s own initial refresh (which is not guaranteed to run synchronously). `applyDock()` already no-ops safely under `prefers-reduced-motion` or before the graph has actually mounted (14 `button.cell`s), so this is safe to call unconditionally on every init.
- `hero/mount.ts`: the deeper cause of the worst-case timing was that the docking-trigger setup (the `.eg-embed` `IntersectionObserver` / first-scroll listener) only started *after* `await renderer.init()` — so a slow WebGPU/WebGL2 cold-shader-cache init serialized in front of the `import('./motion')` fetch, sometimes pushing the whole chain past 900ms. Restructured so `preloadMotion()`/`triggerMotion()` are set up and can start fetching `./motion` immediately when `mountHero()` runs, in parallel with `renderer.init()`; `initMotion()` itself (which needs `hero`'s uniforms) still waits for `hero` to exist, reusing the same in-flight import via `startMotion()`.
- Verification: `npx astro check` 0/0/0, `npm run build` 9 routes, `verify:evidence-graph` exit 0 both pages (including `reducedMotion` still clean). New CDP script (`verify-docking-jump.mjs`): loads `/` at 1440×1000, polls only until the hero canvas has a real size (no scrolling), then does ONE `scrollTo` to the docking point and samples canvas/embed opacity every 50ms for 2s, repeated with fresh page loads. Before the `mount.ts` preload fix: 4/5 runs corrected within ~52ms, but one cold-start run took 920ms (still a real, if less frequent, regression). After the preload fix: 10/10 runs across two batches corrected within 51–58ms, comfortably under the 100ms target, with `maxCanvasOpacity(>=100ms) = 0` on every run. The judge's own gradual-scroll scenario (`verify-docking.mjs`) remains green (`canvasOpacity: "0", embedOpacity: "1", cellCount: 14`) and its fixed-delay jump probe still self-corrects by 2000ms with no further scroll.
