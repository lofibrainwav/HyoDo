...
found in 4.21.3 and carries the zero-detection `hyodo init` documentation
correction already landed on main.
...
- Measurement provenance no longer calls a self-measurement green on the word
  ...
  when the virtualenv lived inside the target. Installed copies are now
  compared by their actual `hyodo/` source content rather than by an enclosing
  repository commit.
- Different source stays `SELF_OTHER_CHECKOUT` (`MISMATCH`); unreadable source
  stays `SOURCE_UNOBSERVED`. The first differing path is shown without
  changing the receipt schema.
- Comparison skips only named tooling/cache/editor artifacts, normalizes CRLF
  only for text-like files, keeps arbitrary dotted paths and binary bytes in
  scope, and uses filesystem identity for same-directory checks.
...
- `install_mode` now reports `editable` for editable installs.

### Documentation

- Clarify `hyodo init` zero-detection behavior: no live
  `.hyodo/gates.toml` is created; an inert `.hyodo/gates.toml.example`
  is written and the sampled fallback remains active.
...
evidence was attached and is immutable; see `docs/releases/4.21.2.md`.
